...
public class ImageCache {
    ...
    private var loadingResponses = [NSURL: [(Int, UIImage?) -> Void]]()
    ...
    final func load(url: NSURL, id: Int, completion: @escaping (Int, UIImage?) -> Swift.Void) {
        // Check for a cached image.
        if let cachedImage = image(url: url) {
            DispatchQueue.main.async {
                completion(id, cachedImage)
            }
            return
        }
